...
public class UsersController : ControllerBase
{
    ...
    [HttpDelete("{uuidUser}")]
    [Authorize(Roles = "ROLE_COACH")]
    public async Task<ActionResult<User>> Delete(string uuidUser)
    {
        var user = await _userService.GetByUuidAsync(uuidUser);
        if (user == null) return NotFound();
        ...
        var userCoach = await _db.Users.FindAsync(user.CoachId);

        if (user.UUID == userCoach!.UUID)
        {
        ...
        var messageSends = _db.Messages.Where(ms => ms.SenderId == id);
        if (messageSends.Any())
            _db.RemoveRange(messageSends);

        var messageReceives = _db.Messages.Where(mr => mr.ReceiverId == id);
        if (messageReceives.Any())
            _db.RemoveRange(messageReceives);
